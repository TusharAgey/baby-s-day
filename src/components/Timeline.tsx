...
export const Timeline = ({ events, selectedDate, onDelete }: TimelineProps) => {
  ...
  return (
    <section className="rounded-3xl border border-white/60 bg-white/50 p-5 shadow-glass backdrop-blur-xl dark:border-slate-700 dark:bg-slate-900/60">
      ...
      <div className="max-h-[500px] overflow-y-auto rounded-2xl border border-white/70 bg-white/60 p-2 dark:border-slate-700 dark:bg-slate-800/60">
        <div className="relative" style={{ height: `${timelineHeight}px` }}>
          ...
          {dayEvents.map((event) => {
            const top = toMinutesOfDay(event.timestamp) * pxPerMinute;
            return (
              <article
                key={event.id}
                className="absolute left-14 right-2 rounded-xl border border-white/80 bg-white/90 p-2 shadow-sm dark:border-slate-700 dark:bg-slate-900/90"
                style={{ top: `${Math.max(0, top - 16)}px` }}
              >
